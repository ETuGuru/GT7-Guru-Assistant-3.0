...
### 🇬🇧 English
This application relies on several large external files that need to be downloaded separately:

- **AI Models**: 
  - Large model files (*.gguf) need to be downloaded and placed in the `models/` directory
  - The primary model needed is `gemma-3-4b-it-Q4_K_M.gguf` (approximately 2.5GB)
  - Models can be downloaded from [Hugging Face](https://huggingface.co/models) or other AI model repositories

- **Binary Dependencies**:
  - TensorFlow binaries: Download `_pywrap_tensorflow_internal.pyd` (approximately 920MB) and place it in `lib/tensorflow/python/`
  - Clang binaries: Download `libclang.dll` (approximately 80MB) and place it in `lib/clang/native/`
  - These binaries can be obtained from the official TensorFlow and Clang distribution packages

Installation of these dependencies is required for full functionality. Refer to the documentation in each directory's README.md for detailed download and setup instructions.

### 🇮🇹 Italiano
Questa applicazione dipende da diversi file esterni di grandi dimensioni che devono essere scaricati separatamente:

- **Modelli AI**:
  - I file di modello di grandi dimensioni (*.gguf) devono essere scaricati e posizionati nella directory `models/`
  - Il modello principale necessario è `gemma-3-4b-it-Q4_K_M.gguf` (circa 2,5 GB)
  - I modelli possono essere scaricati da [Hugging Face](https://huggingface.co/models) o altri repository di modelli AI

- **Dipendenze Binarie**:
  - Binari TensorFlow: Scarica `_pywrap_tensorflow_internal.pyd` (circa 920MB) e posizionalo in `lib/tensorflow/python/`
  - Binari Clang: Scarica `libclang.dll` (circa 80MB) e posizionalo in `lib/clang/native/`
  - Questi binari possono essere ottenuti dai pacchetti di distribuzione ufficiali di TensorFlow e Clang

L'installazione di queste dipendenze è necessaria per la piena funzionalità. Consulta la documentazione nel file README.md di ciascuna directory per istruzioni dettagliate su download e configurazione.
